...
ngx_log_t * ngx_log_init(u_char *filename)
{
   ngx_log_t *log;
   ngx_open_file_t     *ngx_log_file;
   
   log = malloc(sizeof(ngx_log_t));
   ngx_log_file = malloc(sizeof(ngx_open_file_t));


    ngx_log_file->fd = ngx_open_file(filename, NGX_FILE_APPEND,
                                    NGX_FILE_CREATE_OR_OPEN,
                                    NGX_FILE_DEFAULT_ACCESS);

    if (ngx_log_file->fd == NGX_INVALID_FILE) {
        printf("[alert] could not open error log file: \"%s\" failed", filename);
...
size_t ngx_strlcpy(char *dst, const char *src, size_t size){
	...
	//if ( n <= 0 ) return 0;
	// size -1 will be copied unlell \0 terminal
	if (n != 0 && --n != 0) {
		do{
			if ( (*d++ = *s++) == 0 ) break;
		}while(--n != 0);
...
char * ngx_strdup(ngx_pool_t *pool, char *text){
	/*char *temp = ngx_palloc(pool, strlen(text)+1);
	if (temp == NULL) return NULL;
	ngx_memcpy(temp, text, strlen(text));
	return temp;
	*/
	return ngx_prealloc(pool, text, strlen(text) + 1);
}

char * ngx_prealloc(ngx_pool_t *pool, char *text, size_t size){
	char *temp = ngx_palloc(pool, size);
	if (temp == NULL) return NULL;
	ngx_memcpy(temp, text, strlen(text));
	return temp;
}
...
char * ngx_nstrdup(ngx_pool_t *pool, char *text, size_t size){
	char *temp = ngx_palloc(pool, size + 1);
	if (temp == NULL) return NULL;
	ngx_memcpy(temp, text, size );
	return temp;
}
...
{
	while ( buf->end < ( buf->last + n ) ) {
		int newsize = 2 * ( buf->end - buf->start );
		int oldlast = buf->last - buf->start;
		buf->start = ngx_prealloc ( pool, buf->start, newsize );
		buf->end = buf->start + newsize;
		buf->pos = buf->start;
		buf->last = buf->start + oldlast;
	}
	strncpy ( buf->last, str, n );
	buf->last += n;
	*( buf->last ) = '\0';
	return buf->last;
}
