...
rcstring *
rcs_create (ngx_pool_t *pool, size_t length)
{
	rcstring *rcs;
	rcs = ngx_palloc (pool, sizeof (rcstring));	/* allocates memory for a struct rcstring */
	if (rcs == NULL)
		return NULL;
	...
	rcs->text = ngx_palloc(pool, (rcs->max + 1) * sizeof (char) );
	if (rcs->text == NULL)
	{
	//we need't to free 
 //free (rcs);
...
rstring_code
rcs_resize (ngx_pool_t *pool, rcstring * rcs, size_t length)
{
	char *temp;
	assert (rcs != NULL);

	temp = ngx_prealloc (pool, rcs->text, sizeof (char) * (length + 1));	/* length plus L'\0' */
	if (temp == NULL)
	{
	//we need't to free 
 //free (rcs);
...
rstring_code
rcs_catcs (ngx_pool_t *pool, rcstring * pre, const char *pos, const size_t length)
{
	size_t pre_length;
	...
	{
		if (rcs_resize (pool, pre, pre_length + length + 5) != RS_OK)
			return RS_MEMORY;
	}
...
rstring_code
rcs_catc (ngx_pool_t *pool, rcstring * pre, const char c)
{
	size_t pre_length;
	...
	if (pre->max <= pre_length)
	{
		...
		if (rcs_resize (pool, pre, pre->max) != RS_OK)
			return RS_MEMORY;
	}
...
char *
rcs_unwrap (ngx_pool_t *pool, rcstring * rcs)
{
	char *out;
	assert (rcs != NULL);
	...
	if (rcs->text == NULL)
		...
		out = ngx_prealloc (pool, rcs->text, sizeof (char) * (strlen (rcs->text) + 1));
...
json_t *
json_new_value (ngx_pool_t *pool, const enum json_value_type type)
{
	json_t *new_object;
	/* allocate memory to the new object */
	new_object = ngx_palloc (pool, sizeof (json_t));
	if (new_object == NULL)
		return NULL;
...
json_t *
json_new_string (ngx_pool_t *pool, const char *text)
{
	json_t *new_object;
	size_t length;
	...
	new_object->text = ngx_palloc (pool, sizeof (char) * length);
...
json_t *
json_new_number (ngx_pool_t *pool, const char *text)
{
	json_t *new_object;
	size_t length;
...
json_t *
json_new_object (ngx_pool_t *pool)
{
	return json_new_value (pool, JSON_OBJECT);
}


json_t *
json_new_array (ngx_pool_t *pool)
{
	return json_new_value (pool, JSON_ARRAY);
}


json_t *
json_new_null (ngx_pool_t *pool)
{
	return json_new_value (pool, JSON_NULL);
}


json_t *
json_new_true (ngx_pool_t *pool)
{
	return json_new_value (pool, JSON_TRUE);
}


json_t *
json_new_false (ngx_pool_t *pool)
{
	return json_new_value (pool, JSON_FALSE);
}
...
enum json_error
json_insert_pair_into_object (ngx_pool_t *pool, json_t * parent, const char *text_label, json_t * value)
{
	...
	label = json_new_string (pool, text_label);
	if (label == NULL)
		return JSON_MEMORY;
...
enum json_error
json_tree_to_string (ngx_pool_t *pool, json_t * root, char **text)
{
	json_t *cursor;
	rcstring *output;
	assert (root != NULL);
      ...
      state1:			/* open value */
	{
		...
		case JSON_STRING:
			...
			if (rcs_catcs (pool, output, cursor->text, strlen (cursor->text)) != RS_OK)
			{
				return JSON_MEMORY;
			}
			if (rcs_catc (pool, output, '\"') != RS_OK)
		...
		case JSON_TRUE:
			/* must not have any children */
			if (rcs_catcs (pool, output, "true", 4) != RS_OK)
			{
				return JSON_MEMORY;
			}
			goto state2;	/* close value */
		...
		case JSON_FALSE:
			/* must not have any children */
			if (rcs_catcs (pool, output, "false", 5) != RS_OK)
			{
				return JSON_MEMORY;
			}
			goto state2;	/* close value */
		...
		case JSON_NULL:
			/* must not have any children */
			if (rcs_catcs (pool, output, "null", 4) != RS_OK)
			{
				return JSON_MEMORY;
			}
			goto state2;	/* close value */
...
char *
json_format_string (ngx_pool_t *pool, char *text)
{
	size_t pos = 0;
	...
	output = rcs_create (pool, strlen (text));
	while (pos < strlen (text))
	{
		...
		case ':':
			rcs_catcs (pool, output, ": ", 2);
			pos++;
			break;

		case ',':
			rcs_catcs (pool, output, ",\n", 2);
			for (i = 0; i < indentation; i++)
			{
				rcs_catc (pool, output, '\t');
			}
	...
	return rcs_unwrap (pool, output);
}


char *
json_escape (ngx_pool_t *pool, char * text)
{
	rcstring *output;
	size_t i, length;
	char buffer[6];
	...
	for (i = 0; i < length; i++)
	{
		...
		{
			rcs_catcs (pool, output, "\\b", 2);
		}
		else if (text[i] == '\f')
		{
			rcs_catcs (pool, output, "\\f", 2);
		}
		else if (text[i] == '\n')
		{
			rcs_catcs (pool, output, "\\n", 2);
		...
		{
			rcs_catc (pool, output, text[i]);
		}
		else if (text[i] < 0x20)
		{
			sprintf(buffer,"\\u%4.4x",text[i]);
			rcs_catcs(pool, output,buffer,6);
		}
		else
		{
			rcs_catc (pool, output, text[i]);
...
int
lexer (ngx_pool_t *pool, char *buffer, char **p, unsigned int *state, rcstring ** text)
{
	assert (buffer != NULL);
	assert (p != NULL);
	assert (state != NULL);
		...
		case 0:	/* Root document */
			{
				...
				case '9':
					*text = rcs_create (pool, 5);
					if (*text == NULL)
						return LEX_MEMORY;
					if (rcs_catc (pool, *text, *(*p - 1)) != RS_OK)
						return LEX_MEMORY;
...
enum json_error
json_parse_fragment (ngx_pool_t *pool, struct json_parsing_info *info, char *buffer)
{
	json_t *temp = NULL;
	...
	while (*info->p != '\0')
	{
		...
		case 0:	/* starting point */
			{
				switch (lexer (pool, buffer, &info->p, &info->lex_state, (rcstring **) & info->lex_text))
				{
		...
		case 1:	/* open object */
			{
				if (info->cursor == NULL)
				{
					if ((info->cursor = json_new_object (pool)) == NULL)
					{
						return JSON_MEMORY;
					}
				}
				else
				{
					...
					if ((temp = json_new_object (pool)) == NULL)
					{
						return JSON_MEMORY;
					}
		...
		case 2:	/* opened object */
			{
				...
				{
				case LEX_STRING:
					if ((temp = json_new_value (pool, JSON_STRING)) == NULL)
						return JSON_MEMORY;
					temp->text = rcs_unwrap (pool, (rcstring *) info->lex_text), info->lex_text = NULL;
					if (json_insert_child (info->cursor, temp) != JSON_OK)
					{
		...
		case 6:	/* label, pos name separator */
			{
				...
				switch (value = lexer (pool, buffer, &info->p, &info->lex_state, (rcstring **) & info->lex_text))
				{
				case LEX_STRING:
					if ((temp = json_new_value (pool, JSON_STRING)) == NULL)
						return JSON_MEMORY;
				...
				case LEX_NUMBER:
					if ((temp = json_new_value (pool, JSON_NUMBER)) == NULL)
						return JSON_MEMORY;
					temp->text = rcs_unwrap (pool, (rcstring *) info->lex_text), info->lex_text = NULL;
					if (json_insert_child (info->cursor, temp) != JSON_OK)
					{
				...
				case LEX_TRUE:
					if ((temp = json_new_value (pool, JSON_TRUE)) == NULL)
						return JSON_MEMORY;
					if (json_insert_child (info->cursor, temp) != JSON_OK)
					{
						/*TODO specify the exact error message */
				...
				case LEX_FALSE:
					if ((temp = json_new_value (pool, JSON_FALSE)) == NULL)
						return JSON_MEMORY;
					if (json_insert_child (info->cursor, temp) != JSON_OK)
					{
						/*TODO specify the exact error message */
				...
				case LEX_NULL:
					if ((temp = json_new_value (pool, JSON_NULL)) == NULL)
						return JSON_MEMORY;
					if (json_insert_child (info->cursor, temp) != JSON_OK)
					{
						/*TODO specify the exact error message */
		...
		case 7:	/* open array */
			{
				if (info->cursor == NULL)
				{
					if ((info->cursor = json_new_array (pool)) == NULL)
					{
						return JSON_MEMORY;
					}
				}
				else
				{
					...
					if ((temp = json_new_array (pool)) == NULL)
					{
						return JSON_MEMORY;
					}
					if (json_insert_child (info->cursor, temp) != JSON_OK)
...
enum json_error json_parse_document (ngx_pool_t *pool, json_t **root, char *text)
{
	enum json_error error;
	...
	jpi = ngx_palloc (pool, sizeof(struct json_parsing_info));
	if(jpi == NULL)
	{
		return JSON_MEMORY;
	}
	...
	error = json_parse_fragment (pool, jpi, text);
 ...
 //free(jpi);
	...
	enum json_error
json_saxy_parse (ngx_pool_t *pool, struct json_saxy_parser_status *jsps, struct json_saxy_functions *jsf, char c)
{
...
/*                      jsps->state = 0;        // redundant*/
		...
		case '0':
			...
			if ((jsps->temp = rcs_create (pool, 5)) == NULL)
			{
				return JSON_MEMORY;
			}
			if (rcs_catc (pool, ((rcstring *) jsps->temp), '0') != RS_OK)
			{
				return JSON_MEMORY;
			}
			break;
		...
		case '9':
			...
			if (rcs_catc (pool, ((rcstring *) jsps->temp), c) != RS_OK)
			{
				return JSON_MEMORY;
			}
			break;
		...
		case '-':
			...
			if (rcs_catc (pool, ((rcstring *) jsps->temp), '-') != RS_OK)
			{
				return JSON_MEMORY;
			}
      ...
      state2:			/* parse string: escaped character */
	{
		...
		case 'u':
			if (!jsps->string_length_limit_reached)
			{
				...
				{
					if (rcs_catc (pool, ((rcstring *) jsps->temp), 'u') != RS_OK)
					{
						return JSON_MEMORY;
					}
				}
      ...
      state22:			/* parse number: start exponent part */
	{
		...
		case '9':
			if (!jsps->string_length_limit_reached)
			{
				if (rcs_length (((rcstring *) jsps->temp)) < JSON_MAX_STRING_LENGTH)
				{
					rcs_catc (pool, ((rcstring *) jsps->temp), c);
				}
				else
				{
					jsps->string_length_limit_reached = 1;
